...
#define SPEAKER_VOLUME 75   // headphones create noise on negative edge if > 75
...
void _speaker_init()
{
    ...
    EVAL_AUDIO_Play((uint16_t *)audio_buffer, AUDIO_BUFFER_SIZE * 8); // Only stereo is supported*/
...
{
    unsigned n;
#ifdef SAMPLE_RATE_DEBUG
    static unsigned prev_micSampCnt = 0;
    static unsigned prev_micPutCnt = 0;
#endif /* SAMPLE_RATE_DEBUG */
    
    //spkrAt2ndHalf = 0;
#ifdef AUDIO_MAL_MODE_CIRCULAR
#endif /* AUDIO_MAL_MODE_CIRCULAR */
    if (_waveType == 1 || _waveType == 2) {
        for (n = (AUDIO_BUFFER_SIZE/2); n < AUDIO_BUFFER_SIZE; ) {
            if (_waveType == 1)
                sine_gen(&n);
            else
                ramp_gen(&n);
...
#ifdef SAMPLE_RATE_DEBUG
    vcp_printf("spkrComplete %u", micSampCnt - prev_micSampCnt);
    vcp_printf(": %u\r\n", micPutCnt - prev_micPutCnt );
    prev_micSampCnt = micSampCnt;
    prev_micPutCnt = micPutCnt;
#endif /* SAMPLE_RATE_DEBUG */
}
