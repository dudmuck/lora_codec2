...
{    
  ...
  {         
    ...
    /* Manage the remaining file size and new address offset: This function 
       should be coded by user (its prototype is already declared in stm32f4_discovery_audio_codec.h) */  
    EVAL_AUDIO_TransferComplete_CallBack((uint32_t)AUDIO_MAL_DMA_STREAM->M0AR, DMA_GetCurrDataCounter(AUDIO_MAL_DMA_STREAM));
  ...
  if (DMA_GetFlagStatus(AUDIO_MAL_DMA_STREAM, AUDIO_MAL_DMA_FLAG_HT) != RESET)
  {
    /* Manage the remaining file size and new address offset: This function 
       should be coded by user (its prototype is already declared in stm32f4_discovery_audio_codec.h) */  
 #ifdef AUDIO_MAL_MODE_NORMAL
    EVAL_AUDIO_HalfTransfer_CallBack((uint32_t)CurrentPos, 0);
#else
    EVAL_AUDIO_HalfTransfer_CallBack((uint32_t)AUDIO_MAL_DMA_STREAM->M0AR, DMA_GetCurrDataCounter(AUDIO_MAL_DMA_STREAM));
#endif
